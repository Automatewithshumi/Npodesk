...
export default function DonorsPage() {
  ...
  const totalDonated = (donorId: string) => donations.filter(d => d.donor_id === donorId).reduce((sum, d) => sum + d.amount, 0);
  ...
  const handleSaveDonor = async (e: React.FormEvent) => {
    ...
    if (!error) { setSuccess('✅ Donor added!'); loadData(orgId); setDonorForm({ name: '', type: 'Individual', email: '', phone: '' }); setTimeout(() => setSuccess(''), 3000); }
    setSaving(false);
  };

  const handleSaveDonation = async (e: React.FormEvent) => {
    e.preventDefault(); setSaving(true);
    const { error } = await supabase.from('donations').insert({ ...donationForm, amount: parseFloat(donationForm.amount), org_id: orgId, donor_id: donationForm.donor_id || null });
    if (!error) { setSuccess('✅ Donation recorded!'); loadData(orgId); setDonationForm({ donor_id: '', amount: '', method: 'EFT', program: 'General fund', notes: '', received_at: new Date().toISOString().split('T')[0] }); setTab('crm'); setTimeout(() => setSuccess(''), 3000); }
    setSaving(false);
  };

  const exportCSV = () => {
    const headers = ['Donor', 'Type', 'Email', 'Total Donated', 'Donations Count'];
    const rows = donors.map(d => [d.name, d.type, d.email || '', `R${totalDonated(d.id).toLocaleString()}`, donations.filter(don => don.donor_id === d.id).length]);
    const csv = [headers, ...rows].map(r => r.join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'donors.csv'; a.click();
  };

  const filtered = donors.filter(d => (!search || d.name.toLowerCase().includes(search.toLowerCase())) && (!typeF || d.type === typeF));
  ...
  return (
    <>
      ...
      {success && <div style={{ background: '#EAF3DE', border: '0.5px solid #b0d890', borderRadius: 8, padding: '10px 16px', marginBottom: 16, fontSize: 13, color: '#27500A', fontWeight: 500 }}>{success}</div>}
          ...
          <div className="two-col">
              ...
              {loading ? <div style={{ padding: '2rem', textAlign: 'center', color: '#888' }}>Loading donors...</div>
                ...
                : <table>
                  <thead><tr>
                    ...
                    <th style={{ width: '20%' }}>Total given</th>
                    <th style={{ width: '16%' }}>Donations</th>
            ...
            {sel ? (
              <div className="card">
                ...
                {donations.filter(d => d.donor_id === sel.id).slice(0, 4).map((d, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: '0.5px solid rgba(0,0,0,0.05)', fontSize: 12 }}>
                    ...
                    <span style={{ fontWeight: 500, color: '#27500A' }}>R{d.amount.toLocaleString()}</span>
                  </div>
                ))}
                <button className="btn btn-primary btn-sm" style={{ width: '100%', justifyContent: 'center', marginTop: 12 }} onClick={() => { setDonationForm({ ...donationForm, donor_id: sel.id }); setTab('form'); }}>
                  + Record new donation
                </button>
              </div>
            ) : (
              <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: 200 }}>
                <div style={{ textAlign: 'center', color: '#aaa' }}><div style={{ fontSize: 32, marginBottom: 8 }}>👆</div><div style={{ fontSize: 13 }}>Click a donor to view their profile</div></div>
              </div>
            )}
          </div>
        </>
      ...
      {tab === 'form' && (
        <div className="two-col">
          <div className="card">
            <div style={{ fontSize: 14, fontWeight: 500, marginBottom: 16 }}>💰 Record a donation</div>
            <form onSubmit={handleSaveDonation}>
              <div className="form-row">
                <div className="form-group"><label className="form-label">Donor</label>
                  <select className="form-input" value={donationForm.donor_id} onChange={e => setDonationForm({ ...donationForm, donor_id: e.target.value })}>
                    <option value="">Select donor or leave blank</option>
                    {donors.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                  </select>
                </div>
                <div className="form-group"><label className="form-label">Amount (ZAR) *</label><input className="form-input" type="number" placeholder="e.g. 5000" value={donationForm.amount} onChange={e => setDonationForm({ ...donationForm, amount: e.target.value })} required /></div>
              </div>
              <div className="form-row">
                <div className="form-group"><label className="form-label">Method</label>
                  <select className="form-input" value={donationForm.method} onChange={e => setDonationForm({ ...donationForm, method: e.target.value })}>
                    {['EFT', 'Cash', 'Food goods', 'In-kind', 'Online'].map(m => <option key={m}>{m}</option>)}
                  </select>
                </div>
                <div className="form-group"><label className="form-label">Date received</label><input className="form-input" type="date" value={donationForm.received_at} onChange={e => setDonationForm({ ...donationForm, received_at: e.target.value })} /></div>
              </div>
              <div className="form-group"><label className="form-label">Allocated to program</label>
                <select className="form-input" value={donationForm.program} onChange={e => setDonationForm({ ...donationForm, program: e.target.value })}>
                  {['General fund', 'Hot meals', 'Food parcels', 'School feeding', 'Elderly care'].map(p => <option key={p}>{p}</option>)}
                </select>
              </div>
              <div className="form-group"><label className="form-label">Notes</label><textarea className="form-input" rows={2} value={donationForm.notes} onChange={e => setDonationForm({ ...donationForm, notes: e.target.value })} placeholder="Reference number, invoice, notes..." /></div>
              <div className="flex-gap">
            ...
            <div style={{ borderTop: '0.5px solid rgba(0,0,0,0.07)', marginTop: 20, paddingTop: 16 }}>
              <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 12 }}>➕ Add new donor</div>
              <form onSubmit={handleSaveDonor}>
                <div className="form-row">
                  <div className="form-group"><label className="form-label">Donor name *</label><input className="form-input" value={donorForm.name} onChange={e => setDonorForm({ ...donorForm, name: e.target.value })} placeholder="e.g. Shoprite Foundation" required /></div>
                ...
                <div className="form-row">
                  <div className="form-group"><label className="form-label">Email</label><input className="form-input" type="email" value={donorForm.email} onChange={e => setDonorForm({ ...donorForm, email: e.target.value })} placeholder="email@example.com" /></div>
                  <div className="form-group"><label className="form-label">Phone</label><input className="form-input" value={donorForm.phone} onChange={e => setDonorForm({ ...donorForm, phone: e.target.value })} placeholder="07x xxx xxxx" /></div>
                </div>
          ...
          <div className="card">
            ...
            {donations.slice(0, 8).map((d, i) => {
              ...
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: '0.5px solid rgba(0,0,0,0.05)' }}>
                  ...
                  <span style={{ fontSize: 12, fontWeight: 500, color: '#27500A' }}>R{d.amount.toLocaleString()}</span>
                </div>
              );
            })}
